...
##########################################DIRECTIONAL GAUSSIAN GENERATOR################
from collections import Counter
angle_received=60
arr1= np.random.normal(loc=angle_received, scale=4 , size=4000)
...
map1[first_values[:,1], first_values[:,0]]= gauss_val# Here +1
...
# dx= math.cos(math.radians(60))
# dy= math.sin(math.radians(60))
# print("dx, dy ", dx, dy)
# print(first_slopes)

# count=1

# while((i>=0 and i<100) and (j>=0 and j<100)):
#     i=round(i + dy)
#     j=round(j + dx)
#     print("newi,j", i,j)
...
# rows, cols=np.where(map1==1.0)
# mean_list=np.array([rows, cols]).T
# new_indexes=[]
...
#     new_indexes.append([i, j+1])
#     new_indexes.append([i+1, j])
# # Convert the list of side indices to np array and remove rows with wrong index value
# new_indexes= np.array(new_indexes)
# new_indexes= new_indexes[~(new_indexes[:,1] <0 ),:]
# new_indexes= new_indexes[~(new_indexes[:,1] >99 ),:]
# new_indexes= new_indexes[~(new_indexes[:,0] <0 ),:]
# new_indexes= new_indexes[~(new_indexes[:,0] >99 ),:]
# # remove duplicates
# # new_indexes= list(set(new_indexes))
# print("new_indexes are :::",new_indexes)
# # new_indexes=new_indexes[(new_indexes[:,0]!=mean_list[:,0] and new_indexes[:,1]!=mean_list[:,1])]
# new_ind=[]
# for [i,j] in new_indexes:
#     if any((mean_list[:]!=[i,j]).all(1)):
#         new_ind.append([i,j])
# print("-------------------------------------------",new_ind)
# new_ind=np.array(new_ind)
# map1[new_ind[:,0], new_ind[:,1]]=0.5 # map updated
