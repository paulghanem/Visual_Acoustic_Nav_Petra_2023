...
static ImVec4 colorForDynCalibrationState(ST::DynamicCalibrationState state) {
    static const ImVec4 red(1.f, 0.f, 0.f, 1.f);
    static const ImVec4 yellow(1.f, 1.f, 0.f, 1.f);
    static const ImVec4 green(0.f, 1.f, 0.f, 1.f);
    switch (state) {
        case ST::DynamicCalibrationState::Stopped:
            return red;
        case ST::DynamicCalibrationState::InProgress:
        case ST::DynamicCalibrationState::Succeeded:
            return green;
        default:
            return red;
    }
...
static void saveFrameToImage(ST::DepthFrame depthFrame, ST::ColorFrame visibleFrame, ST::InfraredFrame infraredFrame) {
	std::string placedFolderPath = "[AppDocuments]/occ/" + ST::formattedStringFromLocaltime(), \
			depthPNGName = "depthFrame.png", visiblePNGName = "visibleFrame.png", \
			infraredPNGName = "infraredFrame.png", depthPLYName = "depthPointCould.ply", \
			depthIntrinsicCSVName = "depthFrameIntrinsics.csv";
    ...
    std::string visiblePNGPath  = ST::resolveSmartPath(placedFolderPath + "/" + visiblePNGName);
    ...
    std::string depthIntrinsicCSVPath = ST::resolveSmartPath(placedFolderPath + "/" + depthIntrinsicCSVName);
    ...
    if (depthFrame.isValid()) {
        ...
        if (depthFrame.intrinsics().isValid()) {
            FILE * pFile;
            pFile = fopen (depthIntrinsicCSVPath.c_str(),"w");
            if (pFile) {
                fprintf(pFile,"width,height,fx,fy,cx,cy,k1,k2,p1,p2,k3\n");
                fprintf(pFile,
                ...
                GuiSupport::log("Error opening file: %s: %s\n", depthIntrinsicCSVPath.c_str(), strerror(errno));
            }
        }
    }

    if (visibleFrame.isValid())
        visibleFrame.saveImageToPngFile(visiblePNGPath.c_str());
...
void Window::renderStreamingScreen() {
    ...
    ImGui::Text("Dynamic Cal status:");
    ST::DynamicCalibrationState dynCalState = _sessionDelegate.lastDynCalibrationState();
    ImGui::TextColored(colorForDynCalibrationState(dynCalState), "%s", ST::CaptureSessionSample::toString(dynCalState));
    ...
    if (_sessionDelegate.occFileWriter().isWriting()) {
        ...
        if (ImGui::Button("Record OCC File", ImVec2(SLIDER_WIDTH, 40))) {
            ...
            if (ST::createDirectories(path)) {
                ...
                _sessionDelegate.occFileWriter().startWritingToFile(ST::resolveSmartPath(outputOCCFilePathAndName).c_str(), _captureSession);
            }
            else {
