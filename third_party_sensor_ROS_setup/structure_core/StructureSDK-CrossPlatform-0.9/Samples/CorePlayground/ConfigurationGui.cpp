...
void Window::renderConfigurationScreen() {
    ...
    ImGui::PopStyleVar();
    ImGui::SameLine();
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(8.f, 8.f + std::round(PlayIcon_height / 2.f - ImGui::GetTextLineHeight() / 2.f)));
    if (ImGui::Button("Load config")) {
        _screen.store(Screen::LoadConfiguration);
        redrawNext();
    }
    ImGui::SameLine();
    if (ImGui::Button("Save config")) {
        _screen.store(Screen::SaveConfiguration);
        redrawNext();
    }
    ImGui::SameLine();
    ...
    ENUM_RADIO_BUTTON("BodyScanning" "##depthrange", &_settings.structureCore.depthRangeMode, ST::StructureCoreDepthRangeMode::BodyScanning);
    ENUM_RADIO_BUTTON("DarkObject" "##depthrange", &_settings.structureCore.depthRangeMode, ST::StructureCoreDepthRangeMode::DarkObject);
    ...
    ImGui::PopItemWidth();
    ImGui::NewLine();
    ImGui::Text("Color demosaic method:");
    ENUM_RADIO_BUTTON("Bilinear" "##demosaic", &_settings.structureCore.demosaicMethod, ST::StructureCoreDemosaicMethod::Bilinear);
    ENUM_RADIO_BUTTON("Edge Aware" "##demosaic", &_settings.structureCore.demosaicMethod, ST::StructureCoreDemosaicMethod::EdgeAware);
    ImGui::Unindent();
    ...
    ImGui::Checkbox("Stream accelerometer", &_settings.structureCore.accelerometerEnabled);
    ImGui::Checkbox("Stream gyroscope", &_settings.structureCore.gyroscopeEnabled);
    ImGui::NewLine();
    ImGui::Indent();
    ImGui::Text("IMU sample rate:");
    ENUM_RADIO_BUTTON("100 Hz" "##imurate", &_settings.structureCore.imuUpdateRate, ST::StructureCoreIMUUpdateRate::AccelAndGyro_100Hz);
    ENUM_RADIO_BUTTON("200 Hz" "##imurate", &_settings.structureCore.imuUpdateRate, ST::StructureCoreIMUUpdateRate::AccelAndGyro_200Hz);
    ENUM_RADIO_BUTTON("400 Hz" "##imurate", &_settings.structureCore.imuUpdateRate, ST::StructureCoreIMUUpdateRate::AccelAndGyro_400Hz);
    ENUM_RADIO_BUTTON("1000 Hz" "##imurate", &_settings.structureCore.imuUpdateRate, ST::StructureCoreIMUUpdateRate::AccelAndGyro_1000Hz);
    ImGui::Unindent();
...
namespace {
    template<typename T> void loadEnumFromJSON(const JSON& json, T& field, const std::string& key, const std::string& stringForEnumValue, T enumValue) {
    ...
    template<typename T> void saveEnumToJSON(JSON& json, const T& field, const std::string& key, const std::string& stringForEnumValue, T enumValue) {
        if (field == enumValue) {
...
static bool loadConfigurationJSON(ST::CaptureSessionSettings& outSettings, const std::string& jsonstr, const ST::CaptureSessionSettings& baseSettings) {
