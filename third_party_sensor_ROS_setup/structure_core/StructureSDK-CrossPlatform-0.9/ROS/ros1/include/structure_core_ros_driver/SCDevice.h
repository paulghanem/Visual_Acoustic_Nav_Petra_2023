...
namespace structure_core
{
	...
	public:
						...
						std::string camera_name)
		{
			...
			if(depth_framerate_ != infrared_framerate_)
			{
				delegate->img_proc_settings.depth_ir_framerate_diff = true;
				delegate->img_proc_settings.framerate_ir_inv = 1.0 / (infrared_framerate_ + 0.1);
				settings.structureCore.infraredFramerate = depth_framerate_;
			}
			else
			...
			sensor_serial = sensor_serial_;
			if(sensor_serial.compare("null") != 0){
				settings.structureCore.sensorSerial = sensor_serial.c_str();
			}
			...
			setNamespace(camera_name);

		}

		void setNewParams(float visible_initial_gain_,
						  ...
						  int dynamic_calibration_mode_)
		{
			...
			{
				setDynParamsOnInit(visible_initial_gain_, visible_initial_exposure_,
							       infrared_initial_exposure_, infrared_initial_gain_, depth_apply_correction);

				setStaticParams(depth_range_mode_, dynamic_calibration_mode_, infrared_auto_exposure_enabled_);
			}
			else
			{
		...
		void setDynParamsOnInit(float visible_initial_gain_,
				  	  	  	  	float visible_initial_exposure_,
								float infrared_initial_exposure_,
								float infrared_initial_gain_,
		...
		void setStaticParams(int depth_range_mode_, int dynamic_calibration_mode_, bool infrared_auto_exposure_enabled_)
		{
