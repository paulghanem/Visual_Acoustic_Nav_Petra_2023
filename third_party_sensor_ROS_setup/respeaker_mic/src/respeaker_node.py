...
# Custom imports 
from respeaker_mic.msg import VZ_AudioData # Will update name
...
RESPEAKER_RATE = rospy.get_param("/respeaker_mic/respeaker_mic", 16000)
RESPEAKER_CHANNELS = rospy.get_param("/respeaker_mic/respeaker_channels", 6) # 6 must flash 6_channels_firmware.bin first
RESPEAKER_WIDTH = rospy.get_param("/respeaker_mic/respeaker_width", 2)
RESPEAKER_INDEX = get_respeaker_device_id()
CHUNK = rospy.get_param("/respeaker_mic/chunk", 1024)
...
class ROSInterface:
    def __init__(self):
        ...
        self.respeaker.set_vad_threshold(5)

        # Publisher for audio data and speaker verification prediction
        self.audio_data_pub = rospy.Publisher("/respeaker/wav_data", numpy_msg(VZ_AudioData), queue_size=10)

        self.p = pyaudio.PyAudio()

        self.stream = self.p.open(rate=RESPEAKER_RATE,
            ...
            if dev:
                while not rospy.is_shutdown():
                    if self.respeaker.is_voice() == 1:
                        ...
                        self.msg.data = a
                        self.audio_data_pub.publish(self.msg)
