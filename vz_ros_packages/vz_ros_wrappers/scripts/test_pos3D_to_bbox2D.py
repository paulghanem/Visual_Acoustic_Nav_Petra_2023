...
class ConvertPos3Dtobbox2D:
    ...
    def __init__(self):
        ...
        self.test_pub = rospy.Publisher(
            '/person_of_interest_3d', Bounding_Box_3D, queue_size=1)
    ...
    def convert_callback(self, ros_rgb_image, ros_depth_image, rgb_camera_info, face_hse_objects, person_hse_objects):
        ...
        if(len(stamped_frames_person.bounding_boxes) > 0):
            self.test_pub.publish(stamped_frames_person.bounding_boxes[0])
            poi_to_publish = stamped_frames_person.bounding_boxes[0]
            poi_to_publish.header = stamped_frames_person.header
            self.test_pub.publish(stamped_frames_person.bounding_boxes[0])
...
if __name__ == '__main__':
    try:
        rospy.init_node('test_pos3d_to_pixel', anonymous=True)
        pos3d_to_pixel = ConvertPos3Dtobbox2D()
        rospy.spin()

    except rospy.ROSInterruptException:
        print('Tester for converter of pos 3D to 2D pixel failed!')
